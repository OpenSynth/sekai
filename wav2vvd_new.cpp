...
void DestroyMemory(WorldParameters *world_parameters) {
  ...
  for (int i = 0; i < world_parameters->f0_length; ++i) {
    ...
    delete[] world_parameters->mel_cepstrum1[i];
    delete[] world_parameters->mel_cepstrum2[i];
  }
  delete[] world_parameters->spectrogram;
  delete[] world_parameters->aperiodicity;
  delete[] world_parameters->mel_cepstrum1;
  delete[] world_parameters->mel_cepstrum2;
}
...
int main(int argc, char *argv[]) {
  ...
  //check for errors
  DisplayInformation(fs, x_length);
  ...
  world_parameters.cepstrum_length = 32;
  ...
  Compress(world_parameters.spectrogram,world_parameters.aperiodicity,world_parameters.f0_length,fs,world_parameters.fft_size,
		world_parameters.cepstrum_length,world_parameters.mel_cepstrum1,world_parameters.mel_cepstrum2);
  ...
  hdr.version=0;
  hdr.f0_length = world_parameters.f0_length;
  hdr.frame_period = world_parameters.frame_period;
  hdr.cepstrum_length = world_parameters.cepstrum_length;
  hdr.flags=0;
  hdr.fs=fs;
  ...
  for(int j=0;j<world_parameters.f0_length;j++)
    {
      chunk[0] = noteFromFrequency(world_parameters.f0[j]);
      for(int i=0;i<world_parameters.cepstrum_length;i++)
	{
	  chunk[i+1] = world_parameters.mel_cepstrum1[j][i];
	  chunk[i+1+world_parameters.cepstrum_length] = world_parameters.mel_cepstrum2[j][i];
	}
